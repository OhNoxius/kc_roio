...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:key name="artists" match="artist" use="string()"/>
	<!--	<xsl:strip-space elements="artist"/>-->

	<xsl:template match="/">
		<div id="resultsTable">
			<xsl:apply-templates
				select="//artist[generate-id() = generate-id(key('artists', string())[1])]">
		...
		<table id="{generate-id(.)}" summary="{string(current())}" style="width: 100%" border="0"
			...
			<tr style="background-color:#EEEEEE;height:10px;" class="artistClass">
				...
				<td bgcolor="#EEEEEE" align="right">
					<input id="lnk_{generate-id(.)}" type="button" value="[+] Expand"
						style="border:none;background:none;width:85px;"/>
			...
			<tr>
				<td colspan="3" id="tab_{generate-id(.)}" data-calc="false" style="display:none;FONT-SIZE:100%;">					
				</td>
			</tr>
			<tr>
				<td colspan="3"/>
